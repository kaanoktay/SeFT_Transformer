...
class SeqAttentionBlock(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.Wq = tf.Variable(
            initial_value=w_init(
                shape=(num_mod, input_dim, self.proj_dim), dtype="float32"),
            trainable=True
        )
        self.Bq = tf.Variable(
            initial_value=b_init(
    ...
    def call(self, inp, mask):
        ...
        q = tf.einsum('...d,...dp->...p', inp, self.Wq, optimize='branch-all') + \
            self.Bq  # (b, t, m, p)
        k = tf.einsum('...d,...dp->...p', inp, self.Wk, optimize='branch-all') + \
            self.Bk  # (b, t, m, p)
        v = tf.einsum('...d,...dp->...p', inp, self.Wv, optimize='branch-all') + \
            self.Bv  # (b, t, m, p)
        # Separate heads
