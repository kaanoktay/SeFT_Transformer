...
class posEncodingBlock(layers.Layer):
  def __init__(self, enc_dim=128):
    ...
    self.f = tf.math.exp((tf.range(start=0, limit=enc_dim, delta=2, dtype="float32") * -(tf.math.log(10000.0) / enc_dim)))
...
class inpEncodingBlock(layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.W = tf.Variable(
        initial_value=w_init(shape=(num_mod, input_dim, self.enc_dim), dtype="float32"),
        trainable=True
    )
    self.B = tf.Variable(
        initial_value=b_init(shape=(num_mod, self.enc_dim), dtype="float32"),
        trainable=True
    )
  ...
  def call(self, inp):
    ...
    inp_enc = tf.einsum('...i,...id->...d', inp, self.W) + self.B # (b, t, m, d)
    return inp_enc
...
class seqAttentionBlock(layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.Wq = tf.Variable(
        initial_value=w_init(shape=(num_mod, input_dim, self.proj_dim), dtype="float32"),
        trainable=True
    ) 
    self.Bq = tf.Variable(
        initial_value=b_init(shape=(num_mod, self.proj_dim), dtype="float32"),
        trainable=True
    )
  ...
  def call(self, inp, mask):
    ...
    q = tf.einsum('...d,...dp->...p', inp, self.Wq) + self.Bq # (b, t, m, p)
    k = tf.einsum('...d,...dp->...p', inp, self.Wk) + self.Bk # (b, t, m, p)
    v = tf.einsum('...d,...dp->...p', inp, self.Wv) + self.Bv # (b, t, m, p)
    # Separate heads
    q = rearrange(q, 'b t m (h e) -> b m h t e', h=self.num_head) # (b, m, h, t, e)
    k = rearrange(k, 'b t m (h e) -> b m h t e', h=self.num_head) # (b, m, h, t, e)
    v = rearrange(v, 'b t m (h e) -> b m h t e', h=self.num_head) # (b, m, h, t, e)
    # Calculate attention
    score = tf.einsum('...ij,...kj->...ik', q, k) / np.sqrt(self.embed_dim) # (b, m, h, t, t)
    if mask is not None:
    ...
    # Concatenate heads
    concat_output = rearrange(output, 'b m h t e -> b t m (h e)') # (b, t, m, p)
    return concat_output

class modAttentionBlock(layers.Layer):
  def __init__(self, proj_dim=128, num_head=4):
    super(modAttentionBlock, self).__init__()
    self.proj_dim  = proj_dim
    self.num_head  = num_head
    self.embed_dim = proj_dim // num_head
  ...
  def call(self, inp, mask=None):
    ...
    # Separate heads
    q = rearrange(q, 'b t m (h e) -> b t h m e', h=self.num_head) # (b, t, h, m, e)
    k = rearrange(k, 'b t m (h e) -> b t h m e', h=self.num_head) # (b, t, h, m, e)
    v = rearrange(v, 'b t m (h e) -> b t h m e', h=self.num_head) # (b, t, h, m, e)
    # Calculate attention
    score = tf.einsum('...ij,...kj->...ik', q, k) / np.sqrt(self.embed_dim) # (b, t, h, m, m)
    if mask is not None:
    ...
    # Concatenate heads
    concat_output = rearrange(output, 'b t h m e -> b t m (h e)') # (b, t, m, p)
    return concat_output

class axialMultiHeadAttentionBlock(layers.Layer):
  def __init__(self, proj_dim=128, enc_dim=128, num_head=4):
    super(axialMultiHeadAttentionBlock, self).__init__()
    self.seqAttention = seqAttentionBlock(proj_dim=proj_dim, num_head=num_head)
    self.modAttention = modAttentionBlock(proj_dim=proj_dim, num_head=num_head)
    self.enc_dim = enc_dim
  ...
  def call(self, inp, mask):
    ...
    out = tf.einsum('...d,...dp->...p', out, self.W) + self.B # (b, t, m, d)
    return out

class posFeedforwardBlock(layers.Layer):
  def __init__(self, enc_dim=128, ff_dim=128):
    super(posFeedforwardBlock, self).__init__()
    self.enc_dim = enc_dim
    self.ff_dim = ff_dim
  ...
  def build(self, input_shape):
    ...
    self.W1 = tf.Variable(
        initial_value=w_init(shape=(num_mod, input_dim, self.ff_dim), dtype="float32"),
        trainable=True
    )
    self.B1 = tf.Variable(
        initial_value=b_init(shape=(num_mod, self.ff_dim), dtype="float32"),
        trainable=True
    )
    # Weight matrix and bias: 2nd feedforward layer
    self.W2 = tf.Variable(
        initial_value=w_init(shape=(num_mod, self.ff_dim, self.enc_dim), dtype="float32"),
        trainable=True
    )
    self.B2 = tf.Variable(
        initial_value=b_init(shape=(num_mod, self.enc_dim), dtype="float32"),
  ...
  def call(self, inp):
    ...
    out = tf.einsum('...p,...pf->...f', inp, self.W1) + self.B1 # (b, t, m, f)
    out = self.relu(out)
    out = tf.einsum('...f,...fd->...d', out, self.W2) + self.B2 # (b, t, m, d)
    return out
